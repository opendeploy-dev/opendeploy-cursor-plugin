...
| **Worker deployment builds but fails readiness because it listens on no port** | service type/start command + runtime logs | Do not expose the worker as the public app. Keep it as `worker`/internal, and if the current platform still requires an HTTP readiness port, ask before adding a tiny health shim that runs the worker plus `200 ok` on the configured readiness port. |
