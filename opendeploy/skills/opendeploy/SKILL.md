...
6.25. **Resolve AI API choice.** If `analyze`, `preflight`, or `deploy plan` returns `ai_config.requires_ai_api_key: true` with non-empty `api_key_vars`, or direct source review finds AI SDK packages/provider env keys, load `references/ai-api.md`. Show the user the detected providers and exact env key names, then ask whether to use OpenDeploy AI API. The recommended option is `Use OpenDeploy AI API`, which sets each detected AI API key runtime var to `{{MINIONS_AI_API_KEY}}` and each paired AI base URL runtime var to `https://api.opendeploy.dev/v1`; the backend provisions the real project token and stores it securely. Other options are `Use my own AI keys` and `Continue without AI` only when source evidence says the app can boot and smoke-test without AI calls. Do not ask the user to paste OpenAI/Gemini/Anthropic/etc. keys before offering the OpenDeploy AI API option. Do not put `{{MINIONS_AI_API_KEY}}` in `build_variables`: the backend expansion contract runs on runtime env saves/service runtime variables. If the app truly needs an AI key during build, ask for user-provided build-time values or patch the build to avoid provider calls at build time.
